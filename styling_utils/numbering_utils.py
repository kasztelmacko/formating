    ...
    if common_pattern:
        ...
        if not any(char in expanded_pattern for char in ['\\', '(', ')', '[', ']']):
            if common_pattern_side.upper() == "LEFT":
                new_numbering = f"{expanded_pattern}{common_pattern_separator}{new_numbering}"
            else:
                new_numbering = f"{new_numbering}{common_pattern_separator}{expanded_pattern}"
        else:
            pass
...
def process_paragraph_text(text, numbering_format, numbering_side, regex_patterns, separator=" "):
    ...
    if style_definitions and style_attributes_names_mapping and style_name:
        ...
        if style_def:
            ...
            if not common_pattern:
                common_pattern_def = style_def.get(style_attributes_names_mapping.get("common_pattern_format", "common_pattern_format"), {})
...
def apply_chapter_based_numbering(doc, style_names_mapping, style_definitions=None, 
    ...
    for paragraph in doc.paragraphs:
        ...
        if target_styles and style_name in [style_names_mapping.get(style) for style in target_styles]:
            if in_chapter:
                target_style = None
                ...
                if target_style:
                    ...
                    style_def = style_definitions.get(target_style, {}) if style_definitions else {}
                    numbering_def = style_def.get(style_attributes_names_mapping.get("numbering_format", "numbering_format"), {})
